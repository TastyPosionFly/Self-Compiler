...
private:
    ...
    struct symbol_hash {
        std::size_t operator()(const Symbol &s) const {
            return std::hash<std::string>{}(s.name) ^ std::hash<int>{}(static_cast<int>(s.type));
        }
    };

    std::unordered_map<Symbol, std::set<Symbol>, symbol_hash> firstCache;
    std::unordered_map<Symbol, std::set<Symbol>, symbol_hash> followCache;
...
public:
    ...
    void generateParser(string& filename){
    ...
    map<pair<int, Symbol>, pair<char, int>> getActionTable() {
        return actionTable;
    }
